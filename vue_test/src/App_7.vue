<template>
    ...
    methods: {
      handleDownload() {
        import("@/vendor/Export2Excel").then((excel) => {
            ...
            for (const key in this.tableScoreColumn) {
              if (j == 0) {
                if (i == this.evaluators.length) {
                  ...
                  multiHeader.push(this.evaluators[i].cname);
                }
              } else {
                multiHeader.push("");
              }
          ...
          //     multiHeader[i + 3] = "合计";
          //   } else {
          //     multiHeader[i + 3] = this.evaluators[i].cname;
          //   }
          // }
  ...
  export default {
    data() {
      return {
        students: [
          { name: 'Alice', mathScore: 90, englishScore: 85 },
          { name: 'Bob', mathScore: 80, englishScore: 90 },
          { name: 'Charlie', mathScore: 95, englishScore: 80 }
        ]
      }
    },
    methods: {
      downloadTable() {
        ...
        const fileName = 'students_scores.xls'
